(function(w){
  var handLock = function(option){
    ...
    this.dom = {
      info: option.info,
      reset_passwords: option.reset_passwords
    }
  ...
  handLock.prototype = {
    ...
    createListener: function(){ // 创建监听事件
      ...
      this.dom.reset_passwords.addEventListener('click', function(e){
        self.lsPass.model = 2;
        localStorage.clear();
    ...
    checkPass: function(){ // 判断当前 model 和检查密码
      ...
      if(model == 2){// 设置密码
        if(this.touchCircles.length < 5){ // 验证密码长度
          succ = false;
          this.showInfo(lg('密码长度至少为 5!','Password needs to five'), 1000);
        }else{
          succ = true;
          ...
          this.lsPass.model = 3;
          this.showInfo(lg('请再次输入密码','Re-enter the password'), 1000);
        }
        ...
        if(!flag){
          succ = false;
          this.showInfo(lg('两次密码不一致，请重新输入','Twice inconsistent,Again'), 1000);
          ...
          this.showInfo(lg('密码设置成功','Password set up successfully'), 1000);
          this.lsPass.model = 1; 
        ...
        if(!flag){
          succ = false;
          this.showInfo(lg('很遗憾，密码错误','Sorry, the password mistake'), 1000);
        }else{
          succ = true;
          this.showInfo(lg('恭喜你，验证通过','Verification passed'), 1000);
